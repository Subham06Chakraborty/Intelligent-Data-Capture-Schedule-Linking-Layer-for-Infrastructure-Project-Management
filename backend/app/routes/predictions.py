...
# Reads from progress_events (AI-extracted events), not baseline activities
# ─────────────────────────────────────────────────────────────────────────────
@router.get("/delay-prediction", summary="Predict delay risk for a specific progress event")
async def delay_prediction(event_id: str = Query(...)):
    db = get_db()
    ...
    event = doc.to_dict()
    result = predict_delay_risk(event)
    db.collection("progress_events").document(event_id).update({"prediction": result})
